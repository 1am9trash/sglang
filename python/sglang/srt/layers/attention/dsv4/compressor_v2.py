...
class CompressorBackendMixin:
    ...
    def forward_unified(
        ...
        if _is_hip and not envs.SGLANG_OPT_USE_JIT_NORM.get():
            # Non-JIT HIP fallback (Triton). atom_paged: core compressor store is
            ...
            bf16_store = False
            atom_core = is_atom_paged() and not compressor.is_in_indexer
            if atom_core:
                # atom_paged core compressor: write the bf16 compressed KV straight
                # into the unified_kv compress region (slot = swa_pages + out_loc),
                # via the fused all-in-one kernel's bf16-store mode (page_size=1).
                kv_cache = token_to_kv_pool.get_atom_unified_kv(layer_id)
                page_size = 1
                out_loc = out_loc + token_to_kv_pool.atom_swa_pages
                bf16_store = True
    ...
    def _forward_unified_hip(
        ...
        if is_atom_paged() and not is_indexer:
            # atom_paged: the core compressor produces the MAIN bf16 compressed
            ...
            from sglang.srt.layers.attention.dsv4.atom_paged.runtime import (
            ...
            store_compressed_into_unified(
                ...
                unified_kv=token_to_kv_pool.get_atom_unified_kv(layer_id),
                swa_pages=token_to_kv_pool.atom_swa_pages,
            )
            return
