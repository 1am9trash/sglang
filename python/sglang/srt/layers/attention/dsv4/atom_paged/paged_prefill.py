...
class _Envs:
    # Shim for atom.utils.envs (only the two flags the vendored kernels read).
    @property
    def ATOM_FORCE_ATTN_TRITON(self) -> bool:
        # Default True for first bring-up: use the Triton paged-prefill kernel
        # rather than the OPUS gfx950 path.
        return os.environ.get("ATOM_FORCE_ATTN_TRITON", "1") == "1"

    @property
    def ATOM_FUSED_COMPRESS_USE_FLYDSL(self) -> bool:
        return os.environ.get("ATOM_FUSED_COMPRESS_USE_FLYDSL", "0") == "1"
...
def sparse_attn_v4_paged_prefill_reference(
    ...
    from sglang.srt.layers.attention.dsv4.atom_paged._reference import (
