"""Runtime glue for the atom_paged backend.

Builds ATOM-style flat ``kv_indices`` / ``kv_indptr`` from SGLang's already-computed
DSV4 metadata, scatters SWA K into the bf16 ``unified_kv`` ring, and dispatches the
vendored ATOM paged decode/prefill kernels.
...
``*_page_indices`` rows (the same contract the non-atom flashmla path relies
...
from sglang.srt.layers.attention.dsv4.atom_paged.paged_decode import (
    sparse_attn_v4_paged_decode,
)
from sglang.srt.layers.attention.dsv4.atom_paged.paged_prefill import (
    sparse_attn_v4_paged_prefill,
)
from sglang.srt.layers.attention.dsv4.atom_paged.store import _scatter_rows
...
def _swa_scatter_kernel(
    ...
    """One program per token: compute ring slot = state_slot*win + pos%win and
...
def store_swa_into_unified(
    *,
    ...
    During prefill a chunk can contain >win tokens of one request; tokens win apart
    ...
    consecutive positions -> distinct ring slots -> race-free. For decode
...
# ---------------------------------------------------------------------------
# Amortized decode index build (once per forward, ATOM-style 3 streams).
#
...
# ---------------------------------------------------------------------------
from sglang.srt.layers.attention.dsv4.atom_paged.paged_decode_indices import (
...
def build_decode_streams(
    *,
    ...
    ``write_v4_paged_decode_indices`` (cs == win: ring stride == window).
    """
    ...
    if N > 0:
        ...
        # (decode is 1 token/seq); cs == win (no MTP spec ring extension).
...
def _build_prefill_indices_kernel(
    ...
    # ---- prefix SWA: prior-chunk ring slots ----
    for off in tl.range(0, win, BLOCK):
        k = off + tl.arange(0, BLOCK)
        m = k < prefix_swa_count
        gp = swa_low + k
        tl.store(pre_out_ptr + pbase + k, s * win + (gp % win), mask=m)
